...
Wujiang Xu 1 Zujie Liang 2 Kai Mei 1 Hang Gao 1 Juntao Tan 3 Yongfeng Zhang 1 † † thanks: Corresponding Email: yongfeng.zhang@rutgers.edu. 1 Rutgers University 2 Ant Group 3 Salesforce Research
...
Building upon the Zettelkasten method’s principles of atomic note-taking and flexible organization, we introduce an LLM-driven approach to memory note construction. When an agent interacts with its environment, we construct structured memory notes that capture both explicit information and LLM-generated contextual understanding. Each memory note m i subscript 𝑚 𝑖 m_{i} italic_m start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT in our collection ℳ = { m 1 , m 2 , … , m N } ℳ subscript 𝑚 1 subscript 𝑚 2 … subscript 𝑚 𝑁 \mathcal{M}=\{m_{1},m_{2},...,m_{N}\} caligraphic_M = { italic_m start_POSTSUBSCRIPT 1 end_POSTSUBSCRIPT , italic_m start_POSTSUBSCRIPT 2 end_POSTSUBSCRIPT , … , italic_m start_POSTSUBSCRIPT italic_N end_POSTSUBSCRIPT } is represented as:


|  | m i = { c i , t i , K i , G i , X i , e i , L i } subscript 𝑚 𝑖 subscript 𝑐 𝑖 subscript 𝑡 𝑖 subscript 𝐾 𝑖 subscript 𝐺 𝑖 subscript 𝑋 𝑖 subscript 𝑒 𝑖 subscript 𝐿 𝑖 m_{i}=\{c_{i},t_{i},K_{i},G_{i},X_{i},e_{i},L_{i}\} italic_m start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT = { italic_c start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT , italic_t start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT , italic_K start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT , italic_G start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT , italic_X start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT , italic_e start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT , italic_L start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT } |  | (1) |
|---|---|---|---|


where c i subscript 𝑐 𝑖 c_{i} italic_c start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT represents the original interaction content, t i subscript 𝑡 𝑖 t_{i} italic_t start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT is the timestamp of the interaction, K i subscript 𝐾 𝑖 K_{i} italic_K start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT denotes LLM-generated keywords that capture key concepts, G i subscript 𝐺 𝑖 G_{i} italic_G start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT contains LLM-generated tags for categorization, X i subscript 𝑋 𝑖 X_{i} italic_X start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT represents the LLM-generated contextual description that provides rich semantic understanding, and L i subscript 𝐿 𝑖 L_{i} italic_L start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT maintains the set of linked memories that share semantic relationships. To enrich each memory note with meaningful context beyond its basic content and timestamp, we leverage an LLM to analyze the interaction and generate these semantic components. The note construction process involves prompting the LLM with carefully designed templates P s ⁢ 1 subscript 𝑃 𝑠 1 P_{s1} italic_P start_POSTSUBSCRIPT italic_s 1 end_POSTSUBSCRIPT :


|  | K i , G i , X i ← LLM ⁢ ( c i ⁢ ‖ t i ‖ ⁢ P s ⁢ 1 ) ← subscript 𝐾 𝑖 subscript 𝐺 𝑖 subscript 𝑋 𝑖 LLM subscript 𝑐 𝑖 norm subscript 𝑡 𝑖 subscript 𝑃 𝑠 1 K_{i},G_{i},X_{i}\leftarrow\text{LLM}(c_{i}\;\|t_{i}\;\|P_{s1}) italic_K start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT , italic_G start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT , italic_X start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT ← LLM ( italic_c start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT ∥ italic_t start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT ∥ italic_P start_POSTSUBSCRIPT italic_s 1 end_POSTSUBSCRIPT ) |  | (2) |
|---|---|---|---|
...
|  | e i = f enc ⁢ [ concat ⁢ ( c i , K i , G i , X i ) ] subscript 𝑒 𝑖 subscript 𝑓 enc delimited-[] concat subscript 𝑐 𝑖 subscript 𝐾 𝑖 subscript 𝐺 𝑖 subscript 𝑋 𝑖 e_{i}=f_{\text{enc}}[\;\text{concat}(c_{i},K_{i},G_{i},X_{i})\;] italic_e start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT = italic_f start_POSTSUBSCRIPT enc end_POSTSUBSCRIPT [ concat ( italic_c start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT , italic_K start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT , italic_G start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT , italic_X start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT ) ] |  | (3) |
|---|---|---|---|


By using LLMs to generate enriched components, we enable autonomous extraction of implicit knowledge from raw interactions. The multi-faceted note structure ( K i subscript 𝐾 𝑖 K_{i} italic_K start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT , G i subscript 𝐺 𝑖 G_{i} italic_G start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT , X i subscript 𝑋 𝑖 X_{i} italic_X start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT ) creates rich representations that capture different aspects of the memory, facilitating nuanced organization and retrieval. Additionally, the combination of LLM-generated semantic components with dense vector representations provides both human-interpretable context and computationally efficient similarity matching.
...
After creating links for the new memory, A-Mem evolves the retrieved memories based on their textual information and relationships with the new memory. For each memory m j subscript 𝑚 𝑗 m_{j} italic_m start_POSTSUBSCRIPT italic_j end_POSTSUBSCRIPT in the nearest neighbor set ℳ near n superscript subscript ℳ near 𝑛 \mathcal{M}_{\text{near}}^{n} caligraphic_M start_POSTSUBSCRIPT near end_POSTSUBSCRIPT start_POSTSUPERSCRIPT italic_n end_POSTSUPERSCRIPT , the system determines whether to update its context, keywords, and tags. This evolution process can be formally expressed as:



|  | m j ∗ ← LLM ⁢ ( m n ⁢ ‖ ℳ near n ∖ m j ‖ ⁢ m j ∥ P s ⁢ 3 ) ← superscript subscript 𝑚 𝑗 LLM conditional subscript 𝑚 𝑛 norm superscript subscript ℳ near 𝑛 subscript 𝑚 𝑗 subscript 𝑚 𝑗 subscript 𝑃 𝑠 3 m_{j}^{*}\leftarrow\text{LLM}(m_{n}\;\|\mathcal{M}_{\text{near}}^{n}\setminus m%
_{j}\;\|m_{j}\;\|P_{s3}) italic_m start_POSTSUBSCRIPT italic_j end_POSTSUBSCRIPT start_POSTSUPERSCRIPT ∗ end_POSTSUPERSCRIPT ← LLM ( italic_m start_POSTSUBSCRIPT italic_n end_POSTSUBSCRIPT ∥ caligraphic_M start_POSTSUBSCRIPT near end_POSTSUBSCRIPT start_POSTSUPERSCRIPT italic_n end_POSTSUPERSCRIPT ∖ italic_m start_POSTSUBSCRIPT italic_j end_POSTSUBSCRIPT ∥ italic_m start_POSTSUBSCRIPT italic_j end_POSTSUBSCRIPT ∥ italic_P start_POSTSUBSCRIPT italic_s 3 end_POSTSUBSCRIPT ) |  | (7) |
|---|---|---|---|



The evolved memory m j ∗ superscript subscript 𝑚 𝑗 m_{j}^{*} italic_m start_POSTSUBSCRIPT italic_j end_POSTSUBSCRIPT start_POSTSUPERSCRIPT ∗ end_POSTSUPERSCRIPT then replaces the original memory m j subscript 𝑚 𝑗 m_{j} italic_m start_POSTSUBSCRIPT italic_j end_POSTSUBSCRIPT in the memory set ℳ ℳ \mathcal{M} caligraphic_M . This evolutionary approach enables continuous updates and new connections, mimicking human learning processes. As the system processes more memories over time, it develops increasingly sophisticated knowledge structures, discovering higher-order patterns and concepts across multiple memories. This creates a foundation for autonomous memory learning where knowledge organization becomes progressively richer through the ongoing interaction between new experiences and existing memories.
...
In each interaction, our A-Mem performs context-aware memory retrieval to provide the agent with relevant historical information. Given a query text q 𝑞 q italic_q from the current interaction, we first compute its dense vector representation using the same text encoder used for memory notes:



|  | e q = f enc ⁢ ( q ) subscript 𝑒 𝑞 subscript 𝑓 enc 𝑞 e_{q}=f_{\text{enc}}(q) italic_e start_POSTSUBSCRIPT italic_q end_POSTSUBSCRIPT = italic_f start_POSTSUBSCRIPT enc end_POSTSUBSCRIPT ( italic_q ) |  | (8) |
|---|---|---|---|



The system then computes similarity scores between the query embedding and all existing memory notes in ℳ ℳ \mathcal{M} caligraphic_M using cosine similarity:



|  | s q , i = e q ⋅ e i | e q | ⁢ | e i | , where ⁢ e i ∈ m i , ∀ m i ∈ ℳ formulae-sequence subscript 𝑠 𝑞 𝑖 ⋅ subscript 𝑒 𝑞 subscript 𝑒 𝑖 subscript 𝑒 𝑞 subscript 𝑒 𝑖 formulae-sequence where subscript 𝑒 𝑖 subscript 𝑚 𝑖 for-all subscript 𝑚 𝑖 ℳ s_{q,i}=\frac{e_{q}\cdot e_{i}}{|e_{q}||e_{i}|},\text{where}\;e_{i}\in m_{i},%
\;\forall m_{i}\in\mathcal{M} italic_s start_POSTSUBSCRIPT italic_q , italic_i end_POSTSUBSCRIPT = divide start_ARG italic_e start_POSTSUBSCRIPT italic_q end_POSTSUBSCRIPT ⋅ italic_e start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT end_ARG start_ARG | italic_e start_POSTSUBSCRIPT italic_q end_POSTSUBSCRIPT | | italic_e start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT | end_ARG , where italic_e start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT ∈ italic_m start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT , ∀ italic_m start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT ∈ caligraphic_M |  | (9) |
|---|---|---|---|
...
|  | ℳ retrieved = { m i | rank ⁢ ( s q , i ) ≤ k , m i ∈ ℳ } subscript ℳ retrieved conditional-set subscript 𝑚 𝑖 formulae-sequence rank subscript 𝑠 𝑞 𝑖 𝑘 subscript 𝑚 𝑖 ℳ \mathcal{M}_{\text{retrieved}}=\{m_{i}|\text{rank}(s_{q,i})\leq k,m_{i}\in%
\mathcal{M}\} caligraphic_M start_POSTSUBSCRIPT retrieved end_POSTSUBSCRIPT = { italic_m start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT | rank ( italic_s start_POSTSUBSCRIPT italic_q , italic_i end_POSTSUBSCRIPT ) ≤ italic_k , italic_m start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT ∈ caligraphic_M } |  | (10) |
|---|---|---|---|
...
For all baselines and our proposed method, we maintain consistency by employing identical system prompts as detailed in Appendix [C](https://arxiv.org/html/2502.12110v1#A3) . The deployment of Qwen-1.5B/3B and Llama 3.2 1B/3B models is accomplished through local instantiation using Ollama 1 1 1 [https://github.com/ollama/ollama](https://github.com/ollama/ollama) , with LiteLLM 2 2 2 [https://github.com/BerriAI/litellm](https://github.com/BerriAI/litellm) managing structured output generation. For GPT models, we utilize the official structured output API. In our memory retrieval process, we primarily employ k 𝑘 k italic_k =10 for top- k 𝑘 k italic_k memory selection to maintain computational efficiency, while adjusting this parameter for specific categories to optimize performance. The detailed configurations of k 𝑘 k italic_k can be found in Appendix [B.4](https://arxiv.org/html/2502.12110v1#A2.SS4) . For text embedding, we implement the all-minilm-l6-v2 model across all experiments.
...
- Ahrens (2017) Sönke Ahrens. 2017. *How to Take Smart Notes: One Simple Technique to Boost Writing, Learning and Thinking* . Amazon. Second Edition.
...
- Dev and Taranjeet (2024) Khant Dev and Singh Taranjeet. 2024. mem0: The memory layer for ai agents. [https://github.com/mem0ai/mem0](https://github.com/mem0ai/mem0) .
...
- Jang et al. (2023) Jihyoung Jang, Minseong Boo, and Hyounghun Kim. 2023. Conversation chronicles: Towards diverse temporal and relational dynamics in multi-session conversations. *arXiv preprint arXiv:2310.13420* .
...
- Kadavy (2021) David Kadavy. 2021. *Digital Zettelkasten: Principles, Methods, & Examples* . Google Books.
...
- Mei et al. (2024) Kai Mei, Zelong Li, Shuyuan Xu, Ruosong Ye, Yingqiang Ge, and Yongfeng Zhang. 2024. Aios: Llm agent operating system. *arXiv e-prints, pp. arXiv–2403* .
...
- Shi et al. (2024) Zeru Shi, Kai Mei, Mingyu Jin, Yongye Su, Chaoji Zuo, Wenyue Hua, Wujiang Xu, Yujie Ren, Zirui Liu, Mengnan Du, et al. 2024. From commands to prompts: Llm-based semantic file system for aios. *arXiv preprint arXiv:2410.11843* .
...
- Wang et al. (2023a) Bing Wang, Xinnian Liang, Jian Yang, Hui Huang, Shuangzhi Wu, Peihao Wu, Lu Lu, Zejun Ma, and Zhoujun Li. 2023a. Enhancing large language model with self-controlled memory framework. *arXiv preprint arXiv:2304.13343* .
...
- Wang et al. (2024) Xingyao Wang, Boxuan Li, Yufan Song, Frank F Xu, Xiangru Tang, Mingchen Zhuge, Jiayi Pan, Yueqi Song, Bowen Li, Jaskirat Singh, et al. 2024. Openhands: An open platform for ai software developers as generalist agents. *arXiv preprint arXiv:2407.16741* .
...
- Xu (2021) J Xu. 2021. Beyond goldfish memory: Long-term open-domain conversation. *arXiv preprint arXiv:2107.07567* .
- Xu et al. (2023) Wujiang Xu, Shaoshuai Li, Mingming Ha, Xiaobo Guo, Qiongxu Ma, Xiaolei Liu, Linxun Chen, and Zhenfeng Zhu. 2023. Neural node matching for multi-target cross domain recommendation. In *2023 IEEE 39th International Conference on Data Engineering (ICDE)* , pages 2154–2166. IEEE.
- Xu et al. (2024a) Wujiang Xu, Qitian Wu, Zujie Liang, Jiaojiao Han, Xuying Ning, Yunxiao Shi, Wenfang Lin, and Yongfeng Zhang. 2024a. Slmrec: empowering small language models for sequential recommendation. *arXiv preprint arXiv:2405.17890* .
- Xu et al. (2024b) Wujiang Xu, Qitian Wu, Runzhong Wang, Mingming Ha, Qiongxu Ma, Linxun Chen, Bing Han, and Junchi Yan. 2024b. Rethinking cross-domain sequential recommendation under open-world assumptions. In *Proceedings of the ACM on Web Conference 2024* , pages 3173–3184.
- Yu et al. (2023a) Wenhao Yu, Hongming Zhang, Xiaoman Pan, Kaixin Ma, Hongwei Wang, and Dong Yu. 2023a. Chain-of-note: Enhancing robustness in retrieval-augmented language models. *arXiv preprint arXiv:2311.09210* .
- Yu et al. (2023b) Zichun Yu, Chenyan Xiong, Shi Yu, and Zhiyuan Liu. 2023b. Augmentation-adapted retriever improves generalization of language models as generic plug-in. *arXiv preprint arXiv:2305.17331* .
- Zhang et al. (2024a) Guibin Zhang, Haonan Dong, Yuchen Zhang, Zhixun Li, Dingshuo Chen, Kai Wang, Tianlong Chen, Yuxuan Liang, Dawei Cheng, and Kun Wang. 2024a. Gder: Safeguarding efficiency, balancing, and robustness via prototypical graph pruning. *arXiv preprint arXiv:2410.13761* .
- Zhang et al. (2024b) Guibin Zhang, Yanwei Yue, Zhixun Li, Sukwon Yun, Guancheng Wan, Kun Wang, Dawei Cheng, Jeffrey Xu Yu, and Tianlong Chen. 2024b. Cut the crap: An economical communication pipeline for llm-based multi-agent systems. *arXiv preprint arXiv:2410.02506* .
- Zhang et al. (2024c) Guibin Zhang, Yanwei Yue, Xiangguo Sun, Guancheng Wan, Miao Yu, Junfeng Fang, Kun Wang, and Dawei Cheng. 2024c. G-designer: Architecting multi-agent communication topologies via graph neural networks. *arXiv preprint arXiv:2410.11782* .
...
|  | F ⁢ 1 = 2 ⋅ precision ⋅ recall precision + recall 𝐹 1 ⋅ 2 ⋅ precision recall precision recall F1=2\cdot\frac{\text{precision}\cdot\text{recall}}{\text{precision}+\text{%
recall}} italic_F 1 = 2 ⋅ divide start_ARG precision ⋅ recall end_ARG start_ARG precision + recall end_ARG |  | (11) |
|---|---|---|---|
...
|  | precision = true positives true positives + false positives precision true positives true positives false positives \text{precision}=\frac{\text{true positives}}{\text{true positives}+\text{%
false positives}} precision = divide start_ARG true positives end_ARG start_ARG true positives + false positives end_ARG |  | (12) |
|---|---|---|---|



|  | recall = true positives true positives + false negatives recall true positives true positives false negatives \text{recall}=\frac{\text{true positives}}{\text{true positives}+\text{false %
negatives}} recall = divide start_ARG true positives end_ARG start_ARG true positives + false negatives end_ARG |  | (13) |
|---|---|---|---|
...
|  | BLEU-1 = B ⁢ P ⋅ exp ⁡ ( ∑ n = 1 1 w n ⁢ log ⁡ p n ) BLEU-1 ⋅ 𝐵 𝑃 superscript subscript 𝑛 1 1 subscript 𝑤 𝑛 subscript 𝑝 𝑛 \text{BLEU-1}=BP\cdot\exp(\sum_{n=1}^{1}w_{n}\log p_{n}) BLEU-1 = italic_B italic_P ⋅ roman_exp ( ∑ start_POSTSUBSCRIPT italic_n = 1 end_POSTSUBSCRIPT start_POSTSUPERSCRIPT 1 end_POSTSUPERSCRIPT italic_w start_POSTSUBSCRIPT italic_n end_POSTSUBSCRIPT roman_log italic_p start_POSTSUBSCRIPT italic_n end_POSTSUBSCRIPT ) |  | (14) |
|---|---|---|---|
...
|  | B ⁢ P = { 1 if ⁢ c > r e 1 − r / c if ⁢ c ≤ r 𝐵 𝑃 cases 1 if 𝑐 𝑟 superscript 𝑒 1 𝑟 𝑐 if 𝑐 𝑟 BP=\begin{cases}1&\text{if }c>r\\
e^{1-r/c}&\text{if }c\leq r\end{cases} italic_B italic_P = { start_ROW start_CELL 1 end_CELL start_CELL if italic_c > italic_r end_CELL end_ROW start_ROW start_CELL italic_e start_POSTSUPERSCRIPT 1 - italic_r / italic_c end_POSTSUPERSCRIPT end_CELL start_CELL if italic_c ≤ italic_r end_CELL end_ROW |  | (15) |
|---|---|---|---|



|  | p n = ∑ i ∑ k min ⁡ ( h i ⁢ k , m i ⁢ k ) ∑ i ∑ k h i ⁢ k subscript 𝑝 𝑛 subscript 𝑖 subscript 𝑘 subscript ℎ 𝑖 𝑘 subscript 𝑚 𝑖 𝑘 subscript 𝑖 subscript 𝑘 subscript ℎ 𝑖 𝑘 p_{n}=\frac{\sum_{i}\sum_{k}\min(h_{ik},m_{ik})}{\sum_{i}\sum_{k}h_{ik}} italic_p start_POSTSUBSCRIPT italic_n end_POSTSUBSCRIPT = divide start_ARG ∑ start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT ∑ start_POSTSUBSCRIPT italic_k end_POSTSUBSCRIPT roman_min ( italic_h start_POSTSUBSCRIPT italic_i italic_k end_POSTSUBSCRIPT , italic_m start_POSTSUBSCRIPT italic_i italic_k end_POSTSUBSCRIPT ) end_ARG start_ARG ∑ start_POSTSUBSCRIPT italic_i end_POSTSUBSCRIPT ∑ start_POSTSUBSCRIPT italic_k end_POSTSUBSCRIPT italic_h start_POSTSUBSCRIPT italic_i italic_k end_POSTSUBSCRIPT end_ARG |  | (16) |
|---|---|---|---|


Here, c 𝑐 c italic_c is candidate length, r 𝑟 r italic_r is reference length, h i ⁢ k subscript ℎ 𝑖 𝑘 h_{ik} italic_h start_POSTSUBSCRIPT italic_i italic_k end_POSTSUBSCRIPT is the count of n-gram i in candidate k, and m i ⁢ k subscript 𝑚 𝑖 𝑘 m_{ik} italic_m start_POSTSUBSCRIPT italic_i italic_k end_POSTSUBSCRIPT is the maximum count in any reference. In QA, BLEU-1 evaluates the lexical precision of generated answers, particularly useful for generative QA systems where exact matching might be too strict.
...
|  | ROUGE-L = ( 1 + β 2 ) ⁢ R l ⁢ P l R l + β 2 ⁢ P l ROUGE-L 1 superscript 𝛽 2 subscript 𝑅 𝑙 subscript 𝑃 𝑙 subscript 𝑅 𝑙 superscript 𝛽 2 subscript 𝑃 𝑙 \text{ROUGE-L}=\frac{(1+\beta^{2})R_{l}P_{l}}{R_{l}+\beta^{2}P_{l}} ROUGE-L = divide start_ARG ( 1 + italic_β start_POSTSUPERSCRIPT 2 end_POSTSUPERSCRIPT ) italic_R start_POSTSUBSCRIPT italic_l end_POSTSUBSCRIPT italic_P start_POSTSUBSCRIPT italic_l end_POSTSUBSCRIPT end_ARG start_ARG italic_R start_POSTSUBSCRIPT italic_l end_POSTSUBSCRIPT + italic_β start_POSTSUPERSCRIPT 2 end_POSTSUPERSCRIPT italic_P start_POSTSUBSCRIPT italic_l end_POSTSUBSCRIPT end_ARG |  | (17) |
|---|---|---|---|



|  | R l = LCS ⁢ ( X , Y ) | X | subscript 𝑅 𝑙 LCS 𝑋 𝑌 𝑋 R_{l}=\frac{\text{LCS}(X,Y)}{|X|} italic_R start_POSTSUBSCRIPT italic_l end_POSTSUBSCRIPT = divide start_ARG LCS ( italic_X , italic_Y ) end_ARG start_ARG | italic_X | end_ARG |  | (18) |
|---|---|---|---|



|  | P l = LCS ⁢ ( X , Y ) | Y | subscript 𝑃 𝑙 LCS 𝑋 𝑌 𝑌 P_{l}=\frac{\text{LCS}(X,Y)}{|Y|} italic_P start_POSTSUBSCRIPT italic_l end_POSTSUBSCRIPT = divide start_ARG LCS ( italic_X , italic_Y ) end_ARG start_ARG | italic_Y | end_ARG |  | (19) |
|---|---|---|---|


where X 𝑋 X italic_X is reference text, Y 𝑌 Y italic_Y is candidate text, and LCS is the Longest Common Subsequence.
...
|  | ROUGE-2 = ∑ bigram ∈ ref min ⁡ ( Count ref ⁢ ( bigram ) , Count cand ⁢ ( bigram ) ) ∑ bigram ∈ ref Count ref ⁢ ( bigram ) ROUGE-2 subscript bigram ref subscript Count ref bigram subscript Count cand bigram subscript bigram ref subscript Count ref bigram \text{ROUGE-2}=\frac{\sum_{\text{bigram}\in\text{ref}}\min(\text{Count}_{\text%
{ref}}(\text{bigram}),\text{Count}_{\text{cand}}(\text{bigram}))}{\sum_{\text{%
...
|  | METEOR = F mean ⋅ ( 1 − Penalty ) METEOR ⋅ subscript 𝐹 mean 1 Penalty \text{METEOR}=F_{\text{mean}}\cdot(1-\text{Penalty}) METEOR = italic_F start_POSTSUBSCRIPT mean end_POSTSUBSCRIPT ⋅ ( 1 - Penalty ) |  | (21) |
|---|---|---|---|



|  | F mean = 10 ⁢ P ⋅ R R + 9 ⁢ P subscript 𝐹 mean ⋅ 10 𝑃 𝑅 𝑅 9 𝑃 F_{\text{mean}}=\frac{10P\cdot R}{R+9P} italic_F start_POSTSUBSCRIPT mean end_POSTSUBSCRIPT = divide start_ARG 10 italic_P ⋅ italic_R end_ARG start_ARG italic_R + 9 italic_P end_ARG |  | (22) |
|---|---|---|---|



|  | Penalty = 0.5 ⋅ ( ch m ) 3 Penalty ⋅ 0.5 superscript ch 𝑚 3 \text{Penalty}=0.5\cdot(\frac{\text{ch}}{m})^{3} Penalty = 0.5 ⋅ ( divide start_ARG ch end_ARG start_ARG italic_m end_ARG ) start_POSTSUPERSCRIPT 3 end_POSTSUPERSCRIPT |  | (23) |
|---|---|---|---|


where P 𝑃 P italic_P is precision, R 𝑅 R italic_R is recall, ch is number of chunks, and m 𝑚 m italic_m is number of matched unigrams. METEOR is valuable for QA evaluation as it considers semantic similarity beyond exact matching, making it suitable for evaluating paraphrased answers.
...
|  | SBERT_Similarity = cos ⁡ ( SBERT ⁢ ( x ) , SBERT ⁢ ( y ) ) SBERT_Similarity SBERT 𝑥 SBERT 𝑦 \text{SBERT\_Similarity}=\cos(\text{SBERT}(x),\text{SBERT}(y)) SBERT_Similarity = roman_cos ( SBERT ( italic_x ) , SBERT ( italic_y ) ) |  | (24) |
|---|---|---|---|



|  | cos ⁡ ( a , b ) = a ⋅ b ‖ a ‖ ⁢ ‖ b ‖ 𝑎 𝑏 ⋅ 𝑎 𝑏 norm 𝑎 norm 𝑏 \cos(a,b)=\frac{a\cdot b}{\|a\|\|b\|} roman_cos ( italic_a , italic_b ) = divide start_ARG italic_a ⋅ italic_b end_ARG start_ARG ∥ italic_a ∥ ∥ italic_b ∥ end_ARG |  | (25) |
|---|---|---|---|


SBERT( x 𝑥 x italic_x ) represents the sentence embedding of text. SBERT Similarity is particularly useful for evaluating semantic understanding in QA systems, as it can capture meaning similarities even when the lexical overlap is low.
...
All hyperparameter k values are presented in Table [5](https://arxiv.org/html/2502.12110v1#A2.T5) . For models that have already achieved state-of-the-art (SOTA) performance with k=10, we maintain this value without further tuning.
...
The prompt template in Note Construction: P s ⁢ 1 subscript 𝑃 𝑠 1 P_{s1} italic_P start_POSTSUBSCRIPT italic_s 1 end_POSTSUBSCRIPT
...
The prompt template in Link Generation: P s ⁢ 2 subscript 𝑃 𝑠 2 P_{s2} italic_P start_POSTSUBSCRIPT italic_s 2 end_POSTSUBSCRIPT
...
The prompt template in Memory Evolution: P s ⁢ 3 subscript 𝑃 𝑠 3 P_{s3} italic_P start_POSTSUBSCRIPT italic_s 3 end_POSTSUBSCRIPT
 You are an AI memory evolution agent responsible for managing and evolving a knowledge base.
 Analyze the the new memory note according to keywords and context, also with their several nearest neighbors memory.
...
Generated on Mon Feb 17 18:34:08 2025 by [L a T e XML ![Mascot Sammy](data:image/png;base64,)
